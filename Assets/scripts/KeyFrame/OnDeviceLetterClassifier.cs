...
namespace LetterDojo.Dictation.OnDevice
{
    ...
    public sealed class OnDeviceLetterClassifier : MonoBehaviour
    {
        ...
        private bool TryBuildInput(Texture2D texture, out float[] tensor, out bool hasWriting)
        {
            ...
            int padX = Mathf.Max(Mathf.RoundToInt(widthRect * Mathf.Clamp01(paddingFraction)), minimumPaddingPixels);
            int padY = Mathf.Max(Mathf.RoundToInt(heightRect * Mathf.Clamp01(paddingFraction)), minimumPaddingPixels);
            ...
            int offsetX = (squareSide - widthRect) / 2;
            int offsetY = (squareSide - heightRect) / 2;

            for (int srcY = 0; srcY < heightRect; srcY++)
            {
                int sourceRow = (minY + srcY) * width;
                int destRow = (offsetY + srcY) * squareSide;
                for (int srcX = 0; srcX < widthRect; srcX++)
                {
                    int destIndex = destRow + offsetX + srcX;
                    int srcIndex = sourceRow + (minX + srcX);
                    if (destIndex >= 0 && destIndex < square.Length && srcIndex >= 0 && srcIndex < grayscale.Length)
                    {
                        square[destIndex] = grayscale[srcIndex];
                    }
                }
            }
        ...
        private static void ResampleSquareToTensor(IReadOnlyList<float> square, int side, float[] tensor)
        {
            ...
            for (int y = 0; y < 28; y++)
            {
                ...
                int y0 = Mathf.Clamp((int)Mathf.Floor(srcY), 0, side - 1);
                int y1 = Mathf.Clamp(y0 + 1, 0, side - 1);
                float fy = srcY - y0;

                for (int x = 0; x < 28; x++)
                {
                    float srcX = (x + 0.5f) * scale - 0.5f;
                    int x0 = Mathf.Clamp((int)Mathf.Floor(srcX), 0, side - 1);
                    int x1 = Mathf.Clamp(x0 + 1, 0, side - 1);
                    float fx = srcX - x0;

                    float top = Mathf.Lerp(square[y0 * side + x0], square[y0 * side + x1], fx);
                    float bottom = Mathf.Lerp(square[y1 * side + x0], square[y1 * side + x1], fx);
